...
class FaceRecord(QWidget, Ui_FaceRecordForm):
    ...
    def search(self):
        ...
        if search_str != '':
            ...
            for i in self.face_data:
                ...
                if search_str.lower() in str(i[6]).lower():
                    search_result = str(i[6])
                    # 设置目标行
    ...
    def make_table(self):
        ...
        for row in self.face_data:
            id = row[0]
            name = row[1]
            student_id = str(row[6])
            face_data = list(map(float, row[3].split('\n')))
            register_time = row[5].split('.')[0]
            face_photo = row[4]
        ...
        self.tableWidget.setColumnWidth(1, 120)  # 设置2列的宽度
        self.tableWidget.setColumnWidth(2, 230)  # 设置3列的宽度
        ...
        for row_data in data_show:
            ...
            #将图片二进制数据改成图片数据格式
            img_new = np.frombuffer(row_data[4], dtype=np.uint8)
